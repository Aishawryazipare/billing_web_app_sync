...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Tax Slab Report</li>
    </ol>
</section>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box" style="border-top: 3px solid #ffffff;border: 2px solid #00ffc3;">
                ...
                <form class="form-horizontal" id="userForm" method="post" action="{{ url('download_tax_slab_report') }}">
                    {{ csrf_field() }}
                    <div class="box-body">
     ...
     i=1;                        
         ...
         if(data!='') {               
          for (var key=0, size=result.length; key<size; key++){
              total=parseFloat(total)+parseFloat(result[key].tax_total);
            var j = -1;
            var r = new Array();
// represent columns as array
                r[++j] ='<tr><td>'+i+'</td></tr>';
                r[++j] ='<tr><td>'+result[key].discount+'</td></tr>';
                r[++j] ='<tr><td>'+result[key].tax_total+'</td></tr>';
         ...
         table.draw();
                            $('#amt').html("<h3>Total Tax Amount:<span id='total_amt'>"+total+"</span></h3>");
                            $(".result").show();
                            }
                    });  
    $('.select2').select2();
